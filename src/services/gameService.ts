...
export class GameService {
  ...
  private async getPaginated(page: number, limit: number): Promise<PaginatedResponse<Game>> {
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 400 && error.response.data?.message?.includes('Expected number, received string')) {
          console.error('🔧 BACKEND FIX NEEDED: Games endpoint validation schema needs z.coerce.number().optional() for page and limit parameters')
        }
      }
      throw error
    }
    ...
    for (const [index, endpoint] of endpoints.entries()) {
      try {
        ...
        console.log(`✅ Game Service: Fallback result processed:`, {
          ...
          hasTeamRelationships: paginatedData.length > 0 && paginatedData[0].homeTeam ? true : false
        })
  ...
  async getPreseasonGames(teamId?: number, seasonYear?: number, preseasonWeek?: number): Promise<Game[]> {
    console.log(`🔍 Game Service: Fetching games by season: ${seasonYear}`+` and week:${preseasonWeek}`)
    try {
      const response = await apiService.get<ApiResponse<Game[]> | Game[]>(
        `${this.endpoint}/filter`, {
          params: {
      ...
      console.error(`❌ Failed to fetch games by season ${seasonYear} and week ${preseasonWeek}:`, error)
      throw error
    }
  }
  ...
  async create(data: Omit<Game, 'id' | 'homeTeam' | 'awayTeam' | 'createdAt' | 'updatedAt'>): Promise<Game> {
  ...
  async update(id: number, data: Partial<Omit<Game, 'id' | 'homeTeam' | 'awayTeam' | 'createdAt' | 'updatedAt'>>): Promise<Game> {
    console.log(`🔍 Game Service: Updating game ${id}:`, data)
    try {
      const response = await apiService.put<ApiResponse<Game> | Game>(`${this.endpoint}/${id}`, data)
  ...
  async populateTeamRelationships(games: Game[]): Promise<Game[]> {
    ...
    try {
      ...
      console.log(`🔍 Game Service: Need to fetch ${teamIds.size} unique teams:`, Array.from(teamIds))
      ...
      console.log(`✅ Game Service: Successfully populated team relationships for ${populatedGames.length} games`)
