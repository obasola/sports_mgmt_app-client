<!-- src/components/prospect/ProspectEditForm.vue -->
...
// Populate form when prospect data is available
watch(prospect, (newProspect) => {
  if (newProspect) {
    Object.keys(form).forEach(key => {
      if (newProspect[key] !== undefined) {
        form[key] = newProspect[key]
      }
    })
  }
}, { immediate: true })
...
onMounted(async () => {
  ...
  // Populate form if prospect is already loaded
  if (prospect.value) {
    Object.keys(form).forEach(key => {
      if (prospect.value[key] !== undefined) {
        form[key] = prospect.value[key]
      }
    })
  }
})
...
const onSubmit = async () => {
  if (!prospect.value?.id) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No prospect selected for editing',
    })
    return
  }
  ...
  try {
    ...
    await prospectStore.update(prospect.value.id, cleanForm)
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Prospect updated successfully',
    })
    router.push(`/prospects/${prospect.value.id}?mode=read`)
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
...
<template>
  <Card class="edit-form">
    <template #title>
      Edit Prospect
      <span v-if="prospect">: {{ prospect.firstName }} {{ prospect.lastName }}</span>
    </template>
