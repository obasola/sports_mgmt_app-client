        ...
        <template #content>
          ...
          <div class="related-sections mt-4">
            <TabView v-if="playerAward.player">
              ...
              <TabPanel header="Player Awards">
                <DataTable :value="playerAwardsArray" responsiveLayout="scroll" v-if="playerAwardsArray && playerAwardsArray.length > 0">
                ...
                <div v-else class="empty-awards-message">
                  <p>No other awards found for this player</p>
                </div>
              </TabPanel>
            </TabView>
          </div>
...
const playerAward = computed(() => playerAwardStore.currentAward)
const loading = computed(() => playerAwardStore.isLoading)
const error = computed(() => playerAwardStore.error)
const isEditing = ref(false)

// Convert the playerAwards result to an array for DataTable
const playerAwardsArray = computed((): PlayerAward[] => {
  if (playerAward.value && playerAward.value.playerId) {
    const awards = playerAwardStore.fetchAwardsByPlayerId(playerAward.value.playerId)
...
const loadPlayerAward = async (id: number) => {
  try {
    await playerAwardStore.getAwardById(id)

    // If the award has a player, fetch the player details
    if (playerAward.value?.playerId) {
      try {
        await playerStore.fetchPlayerById(playerAward.value.playerId)

        // Also fetch other awards for this player
        await playerAwardStore.fetchAwardsByPlayerId(playerAward.value.playerId)
      } catch (error) {
...
const savePlayerAward = async (updatedAward: PlayerAward) => {
  try {
    if (playerAward.value?.id) {
      await playerAwardStore.updateAward(playerAward.value.id, updatedAward)
      toast.add({
        severity: 'success',
        summary: 'Success',
